...
export const GET = async ({ request, platform }: RequestEvent) => {
	try {
		...
		if (!user) {
			return json({ error: 'Invalid authentication token' }, { 
				status: 401,
				headers: {
					'Access-Control-Allow-Origin': '*',
				}
		...
		const validImages = imageResults.filter((img: any): img is NonNullable<typeof img> => 
			img !== null && 
		...
		return json({ error: 'Failed to fetch user images' }, { 
			status: 500,
			headers: {
				'Access-Control-Allow-Origin': '*',
			}
