...
test.describe('Gameplay Fixes - No Auth', () => {
	test('map overlay positioning and layout fixes', async ({ page, viewport }) => {
		...
		if (mapOverlayBox && viewport) {
			...
			console.log(
				`✅ Map overlay stays within bounds: ${mapOverlayBox.height}px height in ${viewport.height}px viewport`
			);
		}
		...
		if ((await smallSubmitButton.isVisible()) && (await smallSubmitButton.isEnabled())) {
			...
			const nextButton = page.locator(
				'.map-overlay .action-buttons button:has-text("Next Round"), .map-overlay .action-buttons button:has-text("View Results")'
			);
			...
			if (mapBox && statsBox && nextButtonBox) {
				...
				console.log(
					`✅ Small map layout: Map -> Result Stats (horizontal) -> Button (container grows, map stays same size)`
				);
			}
			...
			if (scoreBox && distanceBox && badgeBox) {
				...
				console.log(
					`✅ Three-column layout: Score (${scoreBox.x}px) -> Badge (${badgeBox.x}px) -> Distance (${distanceBox.x}px)`
				);
			}
			...
			console.log(
				`Container height with stats: ${containerHeight}px (should be larger than without stats)`
			);

			console.log(
				`✅ Small map result stats correctly positioned, container grows, map maintains size`
			);
		...
		if (smallMapBoxConstraints && viewport) {
			...
			console.log(
				`Map overlay height: ${smallMapBoxConstraints.height}px, max allowed: ${maxAllowedHeight}px`
			);
		...
		if (actionsBoxConstraints) {
			...
			console.log(
				`✅ Small map action buttons container: ${actionsBoxConstraints.height}px height`
			);
		}
	...
	test('small viewport map overlay constraint', async ({ page }) => {
		...
		if (mapOverlayBox) {
			...
			console.log(
				`Small viewport - Map overlay height: ${mapOverlayBox.height}px, expected max: ${expectedMaxHeight}px`
			);
