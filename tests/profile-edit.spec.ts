...
test.describe('Profile Editing', () => {
	test.beforeEach(async ({ page }) => {
		...
		if (modalCount > 0) {
			...
			const closeButtons = page.locator('button:has-text("×"), button[aria-label="close"], .close-button, button:has-text("Close")');
	...
	test('should display edit profile button', async ({ page }) => {
		...
		const editButton = page.locator('button:has-text("Edit Profile"), button[title*="edit"], .edit-profile-btn');
	...
	test('should update display name', async ({ page }) => {
		...
		const nameInput = page.locator('input[name="displayName"], input[placeholder*="name"], input[type="text"]').first();
	...
	test('should close modal when clicking X button', async ({ page }) => {
		...
		// Click X button
		const closeButton = page.locator('button:has-text("×"), button[aria-label="close"], .close-button');
