	...
	test('Verify delete functionality is available for authenticated users', async ({ page }) => {
		...
		if (hasImages) {
			...
			if (deleteButtons > 0) {
				...
				const firstDeleteButton = page.locator('button[title="Delete photo"], button:has-text("🗑️")').first();
			...
			const uploadButton = await page.locator('button:has-text("Upload"), a[href="/upload"]').count();
			if (uploadButton > 0) {
